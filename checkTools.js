...
const toolExe = { meshroom: ["meshroom_batch.exe"], colmap: ["COLMAP.bat"], openMVS: ["Tests.exe", "InterfaceCOLMAP.exe", "DensifyPointCloud.exe", "ReconstructMesh.exe", "RefineMesh.exe", "TextureMesh.exe"] };


function checkFileExistence(type) {

    try {
        const result = toolExe[type].every(exe => existsSync(path.join(process.env[type], exe)));
        return result;
...
async function checkIntegrity(item) {
    ...
    let output = ""
    const command = item === "meshroom" ? typeConfigs[item].command : toolExe[item][0];
    ...
    child.on('close', (code) => {
        ...
        else if (code !== 0 && output.includes("Nothing to compute. You need to set --input or --inputRecursive.") || output.includes("https://colmap.github.io/")) {
            console.log(`Überprüfung von ${item}...`, chalk.green('OK'));
        } else {
...
const downloadPaths = { meshroom: ["COLMAP-3.9.1-windows-cuda.zip", "https://www.fosshub.com/Meshroom.html?dwl=Meshroom-2023.3.0-win64.zip"], colmap: ["COLMAP-3.8-windows-cuda.zip", "https://github.com/colmap/colmap/releases/download/3.8/COLMAP-3.8-windows-cuda.zip"], openMVS: ["OpenMVS_Windows_x64.7z", "https://github.com/cdcseacave/openMVS/releases/download/v2.2.0/OpenMVS_Windows_x64.7z"] };
    ...
    try {
        switch (select) {
            case "Meshroom":
                ...
                spinner.succeed(`${downloadPaths["meshroom"][1]} heruntergeladen and extrahiert`);
                break;
        ...
        spinner.fail("Es ist ein Fehler beim herunterladen der Software aufgetreten: " + error.message);
...
async function extract7Zip(filename) {
    const openMVSPath = path.join(process.cwd(), "tools", filename.replace('.7z', ''));
